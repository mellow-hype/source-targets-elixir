...
#if TP_FEATURE_ETH_DELAY
        athrs27_phy_off(phyAddr);
#endif
#endif
...
void athrs27_phy_off(int phy_addr)
{
	//printf("phy off!!!!!!!!!!!!!!!!!!\n");
	phy_val_saved = s27_rd_phy( phy_addr, ATHR_PHY_CONTROL);
	s27_wr_phy(phy_addr, ATHR_PHY_CONTROL, phy_val_saved|0x0800);
}

void athrs27_phy_on(int phy_addr)
{
	phy_val_saved = s27_rd_phy(phy_addr, ATHR_PHY_CONTROL);
	s27_wr_phy(phy_addr, ATHR_PHY_CONTROL, phy_val_saved&(~0x0800));
	/* it takes a little while for negotiation. */
	mdelay(2000);
}

void athrs27_phy_on_one(int phy_addr)
{
	phy_val_saved = s27_rd_phy( phy_addr, ATHR_PHY_CONTROL);
	s27_wr_phy(phy_addr, ATHR_PHY_CONTROL, phy_val_saved&(~0x0800));
}

void athrs27_phy_on_all()
{
	 int       phyUnit;
	 uint32_t  phyAddr = 0;
	 for (phyUnit=0; phyUnit < ATHR_PHY_MAX; phyUnit++) 
	 {
	 	phyAddr = ATHR_PHYADDR(phyUnit);
		phy_val_saved = s27_rd_phy( phyAddr, ATHR_PHY_CONTROL);
		s27_wr_phy(phyAddr, ATHR_PHY_CONTROL, phy_val_saved&(~0x0800));
	 }
}
#endif
