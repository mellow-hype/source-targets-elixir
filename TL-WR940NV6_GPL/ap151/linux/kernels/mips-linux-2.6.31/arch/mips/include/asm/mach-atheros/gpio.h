...
/* reset default(WPS) button, default is GPIO1 */
...
#else
#define WPS_BUTTON_GPIO		CONFIG_GPIO_RESET_FAC_BIT
#endif
...
/* do config extra wifi led */
#ifdef CONFIG_GPIO_EXTRA_WIFI_LED
#define EXTRA_WIFI_LED_GPIO CONFIG_GPIO_EXTRA_WIFI_LED_BIT
#define EXTRA_WIFI_LED_ON CONFIG_GPIO_EXTRA_WIFI_LED_ON
#define EXTRA_WIFI_LED_OFF (!CONFIG_GPIO_EXTRA_WIFI_LED_ON)
#endif

/* QSS default button, default is None, we use RST&QSS MUX button */
/* QSS(WPS) LED, default is GPIO3 changed by wangbiao 28Jul14 GPIO15->GPIO3 */
...
/* How long the user must press the button before change WIFI status, default is 0 */
#define WIFI_SW_HOLD_TIME		CONFIG_GPIO_WIFI_SW_HOLD_TIME

/* WiFi button's effective value, default is low(0), but WDR4980 is high(1) */
#define WIFI_SW_ON 	CONFIG_GPIO_WIFI_SW_ON
#endif
...
/* only one led for two usb. by zh, 3May13 */
#ifdef CONFIG_GPIO_USB_0_LED_BIT
#ifdef CONFIG_GPIO_USB_1_LED_BIT
#if (CONFIG_GPIO_USB_0_LED_BIT == CONFIG_GPIO_USB_1_LED_BIT)
#define ONE_LED_FOR_TWO_USB
#endif
#endif
#endif
